...
const Index = () => {
  ...
  const [educationList, setEducationList] = useState<types.educationListType>(
    defaults.educationList
  );
  const [skillsList, setSkillsList] = useState<types.skillsListType>(
    defaults.skillsList
  );
  const [awardsList, setAwardsList] = useState<types.awardsListType>(
    defaults.awardsList
  );
  const [experienceList, setExperienceList] =
    useState<types.experienceListType>(defaults.experienceList);
  const [projectList, setProjectList] = useState<types.projectsListType>(
    defaults.projectList
  );
  ...
  const handleSubmit = () => {
    ...
    const headerCopy = JSON.parse(
      form?.dataset.header ? form.dataset.header : "{}"
    ) as types.headerType;
    const educationListCopy = JSON.parse(
      form?.dataset.educationList ? form.dataset.educationList : "[]"
    ) as types.educationListType;
    const skillsListCopy = JSON.parse(
      form?.dataset.skillsList ? form.dataset.skillsList : "[]"
    ) as types.skillsListType;
    const awardsListCopy = JSON.parse(
      form?.dataset.awardsList ? form.dataset.awardsList : "[]"
    ) as types.awardsListType;
    const experienceListCopy = JSON.parse(
      form?.dataset.experienceList ? form.dataset.experienceList : "[]"
    ) as types.experienceListType;
    const projectListCopy = JSON.parse(
      form?.dataset.projectsList ? form.dataset.projectsList : "[]"
    ) as types.projectsListType;
  ...
  return (
    <>
      <div className="flex h-screen justify-between">
        <div className="w-1/2 overflow-auto p-2 pt-0 text-center">
          <Left />
        </div>

        <div className="flex flex-col place-content-center items-center justify-center p-3">
          <Middle />
        </div>
